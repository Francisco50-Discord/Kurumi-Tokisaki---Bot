...
| Orden | Fuente | Uso |
|---|---|---|
| 1 | PurrBot, Nekobot y Nekos.life | Solo endpoints públicos dedicados: `anal`, `blowjob`, `cum`, `hentai` y `yuri`, entre los que están documentados y disponibles. |
...
Todas las rutas usan HTTPS, evitan repeticiones recientes por chat y aplican límites de tamaño. Se descartan animales, contenido furry/anthro/feral, escenas sexuales entre animales, cruces humano-animal, bestialidad, cómics, viñetas, globos de diálogo, animación occidental, variantes `toons`/`famous_toons`, personajes reconocibles de franquicias, contenido sexualizado con menores o personajes de apariencia infantil, contenido sexual no consentido o de cautiverio, tortura, baja calidad, arte de IA, marcas de agua, logos, texto promocional y etiquetas de identidad no solicitadas. Si la consulta preferida no devuelve opciones, solo se amplía dentro de la **misma categoría** y sin retirar exclusiones. Para `ahegao` y `succubus`, el selector usa además un fallback de Xbooru con rotación de páginas; `ahegao` valida su tag explícito y `succubus` acepta el tag equivalente `demon_girl` cuando conserva marcadores femeninos y supera los controles de calidad. Esto evita depender de rutas públicas inexistentes y mantiene ambas categorías separadas. También existe una exclusión por MD5/ID para las imágenes que el usuario marcó, limitada a estas dos categorías, y la memoria por chat usa el hash del archivo cuando la fuente lo proporciona para evitar repeticiones aunque cambie la URL del CDN.

Como regla adicional, se excluyen las escenas de sexo grupal o con múltiples participantes mediante `group_sex`, `gangbang`, `threesome`, `foursome`, `multiple_partners`, `multiple_male` y `polyamory`. También se excluyen `child`, `underage`, `minor`, `preteen`, `toddler`, `lolicon`, `shotacon`, `ageplay`, `aged_down` y `young_looking`, así como franquicias reconocibles y sus personajes —por ejemplo, variantes de Pokémon, Ben 10 y Avatar— y marcas promocionales como `patreon` o `patreon_logo`. **`orgy`, `orgia` y `harem` quedan permitidos** y no se filtran por sí solos. También se excluyen `futanari`/`futa`, `intersex`, `newhalf`, `otoko_no_ko`, `transgender` y variantes de anatomía masculina incompatible en personajes femeninos, incluidos `dickgirl`, `dick_girl`, `shemale`, `penis_on_female`, `female_with_penis` y `female_penile`. También se excluyen `yaoi`, `bl`, `boys_love`, `gay`, `male_on_male`, `male_male`, `boy_on_boy` y `bara`. Se bloquean además `furry`, `anthro`, `feral`, `animal_on_animal`, `animal_sex`, `animal_mating` y etiquetas equivalentes de bestialidad. El filtro rechaza asimismo `rape`, `nonconsensual`, `forced`, `coercion`, `captive`, `captivity`, `prisoner`, `cage`, `torture`, `let_me_go`, `no_means_no` y etiquetas equivalentes. Se excluyen también las variantes `cartoon_character`, `toons`, `famous_toons`, `famous_toons_facial`, `watermark` y material promocional. Asimismo, se excluyen los fetiches `hyper`, `breast_expansion`, `breast_expansion_animation`, `breast_expansion_fetish`, `breast_growth`, `dick_nipples`, `dicknipples`, `phallic_nipples` y `phallic_nipple`. **No se excluyen** `hetero`, `straight`, `male`, `1boy`, `male_on_female`, `breasts`, `large_breasts` ni `huge_breasts` por sí solas, ni la participación de un hombre con una mujer adulta. `bondage`/`shibari` de `/keta` tampoco se bloquea por sí solo cuando no está acompañado de señales de coerción o cautiverio. Una escena heterosexual de una sola pareja adulta sigue permitida; el bloqueo adicional se refiere a múltiples participantes, no a la presencia masculina permitida en una relación hombre-mujer.
...
| `yuri` | `yuri` | Endpoint `yuri` o tags `female_on_female`/`yuri`. |
...
| `succubus`, `thighs` | Su categoría propia | Tags `succubus`/`demon_girl` o `thighhighs`, respectivamente. |
