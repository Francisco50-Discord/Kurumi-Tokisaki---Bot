...
const STRICT_FEMALE_CATEGORIES = new Set(["hentai", "gasm", "ahegao"]);
...
const STRICT_MALE_TAGS = new Set([
  ...
  "penis_in_pussy", "penis_in_ass", "male_penetrating", "male_rape", "male_masturbation"
]);
...
const purrbotEndpoint = (path) => ({ url: `${PURRBOT_API_BASE}/${path}/gif`, field: "link", isExactCategory: true });
const PUBLIC_CATEGORY_APIS = {
  hentai: { endpoints: [{ url: "https://nekobot.xyz/api/image?type=hentai", field: "message", isExactCategory: true }, purrbotEndpoint("solo")] },
  gasm: { endpoints: [{ url: "https://nekos.life/api/v2/img/gasm", field: "url", isExactCategory: true }, purrbotEndpoint("solo")] },
  ahegao: { endpoints: [purrbotEndpoint("solo"), { url: "https://nekobot.xyz/api/image?type=hentai", field: "message", isExactCategory: true }] },
  ...
  boobs: { endpoints: [purrbotEndpoint("solo")] },
  ass: { endpoints: [purrbotEndpoint("solo")] },
  ecchi: { endpoints: [purrbotEndpoint("solo")] },
  erok: { endpoints: [purrbotEndpoint("solo")] },
  hentaigasm: { endpoints: [purrbotEndpoint("solo")] },
  lewd: { endpoints: [purrbotEndpoint("solo")] },
  nsfwwaifu: { endpoints: [purrbotEndpoint("solo")] },
  paizuri: { endpoints: [purrbotEndpoint("solo")] }
};
...
export function isNsfwCandidateAllowed(candidate, category) {
  ...
  const isExactPublicCategory = PUBLIC_CATEGORY_SOURCES.has(candidate?.source) && candidate?.apiCategory === normalizedCategory;
  ...
  if (STRICT_FEMALE_CATEGORIES.has(normalizedCategory)) {
    ...
    if (!isExactCategory) {
      ...
      if (tags.length < STRICT_MIN_TAGS) return false;
      if (!["xbooru", "danbooru"].includes(candidate.source) || Number(candidate.score || 0) < STRICT_MIN_XBOORU_SCORE) return false;
  ...
  const required = CATEGORY_REQUIRED_TAGS[String(category || "").toLowerCase()];
...
export async function getNsfwImageCandidate(category, options = {}) {
  ...
  const preferredRequests = [
    ...
    ...shuffle(preferredFallbackTags).slice(0, 2).flatMap((tags) => [fetchXbooru(tags), fetchYandere(tags), fetchKonachan(tags)])
  ];
  ...
  const broadRequests = [
    ...
    ...shuffle(broadFallbackTags).slice(0, 1).flatMap((tags) => [fetchXbooru(tags), fetchYandere(tags), fetchKonachan(tags)])
  ];
...
export async function downloadMediaBuffer(url, { timeout = IMAGE_SEND_TIMEOUT_MS, maxBytes = MAX_IMAGE_BYTES } = {}) {
  ...
  try {
    const { data } = await axios.get(fixedUrl, {
      ...
      headers: isDanbooruMedia ? undefined : {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
        "Referer": /xbooru/i.test(fixedUrl) ? "https://xbooru.com/" : "https://yande.re/",
