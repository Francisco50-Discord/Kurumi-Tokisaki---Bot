...
function mediaHeadersForUrl(url) {
  ...
  const referer = /gelbooru/i.test(value)
    ? "https://gelbooru.com/"
    : /xbooru/i.test(value)
      ? "https://xbooru.com/"
      : /purrbot/i.test(value)
        ? "https://purrbot.site/"
        : /rule34/i.test(value)
          ? "https://rule34.xxx/"
          : /danbooru|donmai/i.test(value)
            ? undefined
            : undefined;
  const mediaHeaders = {
    ...REQUEST_HEADERS,
  ...
  return referer ? { ...mediaHeaders, Referer: referer } : mediaHeaders;
}
  ...
  try {
    if (Buffer.isBuffer(gifUrlOrBuffer)) {
      ...
      const res = await axios.get(gifUrlOrBuffer, {
        ...
        maxRedirects: 5,
        headers: mediaHeadersForUrl(gifUrlOrBuffer),
      });
      const downloaded = Buffer.from(res.data);
      if (!isGifBuffer(downloaded)) {
        const contentType = String(res.headers?.["content-type"] || "").toLowerCase();
        throw new Error(`respuesta GIF inválida (${downloaded.length} bytes, ${contentType || "sin content-type"})`);
      }
      fs.writeFileSync(inputPath, downloaded);
    }

    if (Buffer.isBuffer(gifUrlOrBuffer) && !isGifBuffer(gifUrlOrBuffer)) {
      throw new Error(`buffer GIF inválido (${gifUrlOrBuffer.length} bytes)`);
    }
    ...
    if (converted.length < 3000) throw new Error(`conversión MP4 vacía (${converted.length} bytes)`);
    return converted;
  } finally {
...
export async function sendAnimeMediaMessage(conn, chatId, mediaUrl, caption, options = {}) {
  ...
  const preloadedBuffer = Buffer.isBuffer(options.preloadedBuffer) ? options.preloadedBuffer : null;
  ...
  if (isGif) {
    try {
      const mp4Buffer = await convertGifToMp4Buffer(preloadedBuffer || mediaUrl);
      ...
      if (options.allowAnimatedFallback === false) throw e;
  ...
  if (isDirectVideo) {
    return await conn.sendMessage(
      chatId,
      { video: preloadedBuffer || { url: mediaUrl }, gifPlayback: true, caption, mimetype: "video/mp4", mentions },
      sendOpts
    );
  }
  ...
  if (preloadedBuffer) {
    return await conn.sendMessage(
      chatId,
      { image: preloadedBuffer, caption, mentions },
      sendOpts
    );
  }
