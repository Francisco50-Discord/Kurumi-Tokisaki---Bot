...
// Resolver la identidad telefónica del bot sin reutilizar por error el JID
// del chat actual. Baileys puede exponerla en distintos campos según la
// versión y el tipo de sesión (id, jid, phoneNumber o creds.me).
export function getBotJid(conn, configuredNumber = "") {
  const candidates = [
    conn?.user?.phoneNumber,
    conn?.user?.id,
    conn?.user?.jid,
    conn?.authState?.creds?.me?.phoneNumber,
  ...
  for (const candidate of candidates) {
    ...
    const raw = typeof candidate === "object"
      ? (candidate.id || candidate.jid || candidate.phoneNumber || candidate.user || "")
      : String(candidate);
    if (!raw) continue;
